...
try {
  ...
  if ($toolset -ieq "Default") {
    $projectPath = "src\NuGet\Qtyi.Net.Compilers.Toolset\AnyCpu\Qtyi.Net.Compilers.Toolset.Package.csproj"
    $packageName = "Qtyi.Net.Compilers.Toolset"
  }
  elseif ($toolset -ieq "Framework") {
    $projectPath = "src\NuGet\Qtyi.Net.Compilers.Toolset\Framework\Qtyi.Net.Compilers.Toolset.Framework.Package.csproj"
    $packageName = "Qtyi.Net.Compilers.Toolset.Framework"
  }
  else {
  ...
  # Because we override the Lua-based languages toolset to build against our LKG package, it is important
  ...
  $args += " /t:Pack /p:LunaEnforceCodeStyle=false /p:DotNetUseShippingVersions=true /p:InitialDefineConstants=BOOTSTRAP"
