...
try {
  ...
  Write-Host "Building Luna"
  & eng/build.ps1 -restore -build -bootstrapDir:$bootstrapDir -ci:$ci -prepareMachine:$prepareMachine -runAnalyzers:$true -configuration:$configuration -pack -binaryLog -useGlobalNuGetCache:$false -warnAsError:$true -properties:"/p:LunaEnforceCodeStyle=true"
  Test-LastExitCode
  ...
  Exec-Command $buildBossPath "-r `"$RepoRoot/`" -c $configuration -p Luna.sln"
  Write-Host ""
  ...
  Test-LastExitCode
  Exec-DotNet "tool run dotnet-format whitespace . --folder --include-generated --include src/**/Generated/ --verify-no-changes"
  Write-Host ""
  ...
  Write-Host "##vso[task.logissue type=error]How to investigate bootstrap failures: https://github.com/qtyi/luna/blob/main/docs/contributing/Bootstrap%20Builds.md#Investigating"
  ExitWithExitCode 1
}
finally {
  if ($enableDumps) {
