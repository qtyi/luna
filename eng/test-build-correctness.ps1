...
try {
  ...
  Exec-Block { & (Join-Path $PSScriptRoot "build.ps1") -restore -build -bootstrap -bootstrapConfiguration:Debug -ci:$ci -runAnalyzers:$true -configuration:$configuration -pack -binaryLog -useGlobalNuGetCache:$false -warnAsError:$true -properties "/p:LunaEnforceCodeStyle=true"}
  ...
  Write-Host "$buildBossPath -r `"$RepoRoot/`" -c $configuration -p Luna.sln"
  Exec-Console $buildBossPath "-r `"$RepoRoot/`" -c $configuration" -p Luna.sln
  Write-Host ""
  ...
  Write-Host "How to investigate bootstrap failures: https://github.com/qtyi/luna/blob/main/docs/compilers/Bootstrap%20Builds.md#Investigating"
  exit 1
}
finally {
  if ($enableDumps) {
