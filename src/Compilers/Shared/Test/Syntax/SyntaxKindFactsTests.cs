...
public partial class SyntaxFactsTests : ThisTestBase
{
    ...
    public static readonly TheoryData<LanguageVersion> EffectiveLanguageVersions = new(
        from LanguageVersion version in Enum.GetValues(typeof(LanguageVersion))
    ...
    #region Keyword
    [Theory]
    [MemberData(nameof(EffectiveLanguageVersions))]
    ...
    [Theory]
    [MemberData(nameof(EffectiveLanguageVersions))]
    public void GetText_Punctuation(LanguageVersion version)
    {
        TestGetTextGetKind(
            version,
            expectedProvider: GetExpectedSyntaxKindInfos_Punctuation);
