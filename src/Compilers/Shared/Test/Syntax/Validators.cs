...
using InternalSyntaxList<T> = Microsoft.CodeAnalysis.Syntax.InternalSyntax.SyntaxList<T> where T : GreenNode;
using Newtonsoft.Json.Linq;

public delegate void NodeValidator(SyntaxKind kind, SyntaxKind? contextualKind = null, string? text = null, object? value = null, TriviaLocation location = TriviaLocation.None);
...
internal readonly struct ValidatorImpl
{
    ...
    public void NextNode(SyntaxKind kind, SyntaxKind? contextualKind, string? text, object? value, TriviaLocation location)
    {
        if (_etor.MoveNext())
        {
            var result = _etor.Current;
            result.Node.Validate(kind, contextualKind, text, value, result.Parent, location);
        }
        else
...
internal static class ValidatorExtensions
{
    public static NodeValidator EndOfFile(this NodeValidator V)
        => (kind, contextualKind, text, value, location) =>
        {
            V(kind, contextualKind, text, value, location);
    ...
    {
        if (node is SyntaxToken token)
            token.Validate(kind, contextualKind, text, value, output, indent);
        else if (node is SyntaxTrivia trivia)
