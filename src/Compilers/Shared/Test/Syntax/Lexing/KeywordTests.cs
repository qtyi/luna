...
{
    [Theory]
    [MemberData(nameof(SyntaxFactsTests.EffectiveLanguageVersions), MemberType = typeof(SyntaxFactsTests))]
    ...
    public void LexContextualKeyword(LanguageVersion version)
    {
        ...
        {
            var V = LexSource(info.Text, options: parseOptions).EndOfFile();V(kind: SyntaxKind.IdentifierToken, contextualKind: info.Kind, text: info.Text);
        }
    }
