...
internal partial class Lexer : AbstractLexer
{
    ...
    /// <summary><see langword="true"/> if we allow preprocessor directives; otherwise, <see langword="false"/>.</summary>
    ...
    public SyntaxToken LexEndOfDirectiveWithOptionalPreprocessingMessage()
    {
        ...
        ScanToEndOfLine(builder: builder.Builder, trimEnd: false);
...
#warning Documentation comment needs globalize.
    private bool ScanLongBrackets(out bool isTerminal, int level = -1)
    {
        ...
        if (level < 0)
        {
            ...
            while (true)
            {
                var c = TextWindow.PeekChar(level + 1);
                if (c == '=') // 优先扫描等号字符。
                {
                    level++;
                    continue;
        ...
        while (true)
        {
            ...
            if (c == SlidingTextWindow.InvalidCharacter && TextWindow.IsReallyAtEnd()) break;
            ...
            if (isPairedLevel && TextWindow.PeekChar() == ']') // 长方括号结构完全配对。
            {
                TextWindow.AdvanceChar();
                // 由于this._builder的末尾处有结束长方括号结构，因此需要删除这段内容。
                _builder.Length -= level + 1;
    ...
    private bool IsPunctuationAvaliable(SyntaxKind kind) => SyntaxFacts.IsPunctuation(kind, _options.LanguageVersion);
