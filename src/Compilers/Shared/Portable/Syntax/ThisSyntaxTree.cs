...
#warning 需要文档注释。
    ...
    /// </para>
    /// <para>
    /// 注意：不应在其他位置调用此方法，若要创建新语法树对象，应使用<see cref="Create(ThisSyntaxNode, ThisParseOptions?, string?, Encoding?)"/>。
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// 语法树只有在调用<see cref="GetRoot(CancellationToken)"/>时才解析。
    /// </remarks>
    ...
    /// <inheritdoc cref="ParseText(SourceText, ThisParseOptions?, string, CancellationToken)"/>
    ...
    private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
    {
        ...
        // 创建词法分析器。
        using var lexer = new Syntax.InternalSyntax.Lexer(newText, Options);
        // 创建语言解析器。
    ...
    private LinePosition GetLinePosition(int position, CancellationToken cancellationToken) => GetText(cancellationToken).Lines.GetLinePosition(position);
    ...
    public sealed override FileLinePositionSpan GetMappedLineSpan(TextSpan span, CancellationToken cancellationToken = default) => GetLineSpan(span, cancellationToken);
    ...
    public override IEnumerable<Diagnostic> GetDiagnostics(CancellationToken cancellationToken = default) => GetDiagnostics(GetRoot(cancellationToken));
    #endregion

    #region SyntaxTree
    /// <inheritdoc/>
    protected sealed override Microsoft.CodeAnalysis.ParseOptions OptionsCore => Options;

    /// <inheritdoc/>
    protected sealed override SyntaxNode GetRootCore(CancellationToken cancellationToken) => GetRoot(cancellationToken);

    /// <inheritdoc/>
    protected sealed override async Task<SyntaxNode> GetRootAsyncCore(CancellationToken cancellationToken) => await GetRootAsync(cancellationToken).ConfigureAwait(false);

    /// <inheritdoc/>
    protected sealed override bool TryGetRootCore([NotNullWhen(true)] out SyntaxNode? root)
    {
        if (TryGetRoot(out var node))
        {
            root = node;
            return true;
        }
