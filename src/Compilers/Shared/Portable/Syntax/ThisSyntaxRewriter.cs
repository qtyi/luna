...
#endif
{
    ...
    public
        ...
        MoonScriptSyntaxRewriter
#endif
        (bool visitIntoStructuredTrivia = false) => _visitIntoStructuredTrivia = visitIntoStructuredTrivia;
    ...
    public virtual SyntaxList<TNode> VisitList<TNode>(SyntaxList<TNode> list) where TNode : SyntaxNode
    {
        ...
        for (int i = 0, n = list.Count; i < n; i++)
        {
            ...
            if (!alternate.IsNull && visited is not null && !visited.IsKind(SyntaxKind.None))
    ...
    public virtual SyntaxTriviaList VisitList(SyntaxTriviaList list)
    {
        ...
        if (count != 0)
        {
            ...
            {
                index++;
                var visited = VisitListElement(trivia);
    ...
    public virtual SyntaxTrivia VisitListElement(SyntaxTrivia trivia) => VisitTrivia(trivia);
}
