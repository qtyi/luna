...
internal static class RealParser
{
    public static bool TryParseHexadecimalDouble(string s, out double d)
    {
        ...
        var decimalSeparatorIndex = exponentIndex; // 默认小数点的位置为指数前缀的位置（即不含小数点和小数部分）
        ulong mantissa = 0;
        var shiftedDigits = 0;
        for (var i = isHex ? 2 : 0; i < exponentIndex; i++)
        {
            ...
            {
                case '.':
                    decimalSeparatorIndex = i; // 查找小数点的位置。
                    continue;
                case 'p':
                case 'P':
                    exponentIndex = i; // 查找指数前缀的位置。
                    /* 小数点的位置（如果有）必定在指数前缀的位置前。
                     * 如果字符串中包含小数点，则decimalSeparatorIndex在循环前期就已经被发现并设置。此时exponentIndex还是默认值，必定比decimalSeparatorIndex大，条件不成立；
                     * 如果到此时仍未找到小数点，则decimalSeparatorIndex还是默认值，必定比exponentIndex大，条件成立。
                     */
                    if (decimalSeparatorIndex > exponentIndex)
                        decimalSeparatorIndex = exponentIndex; // 设置为指数前缀的位置（即不含小数点和小数部分）
                    continue;
            }
        ...
        // 1234.56789 => 123456789.0, decimalSeparatorOffset = decimalSeparatorIndex - exponentIndex. (< 0 because of left shift)
        var offset = (decimalSeparatorIndex - exponentIndex + shiftedDigits) * 4 - 1;
