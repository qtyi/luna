    ...
    where TTreeType : ITreeType
{
    ...
    /// <inheritdoc cref="TreeFileWriter{TTree, TTreeType}.TreeFileWriter(TextWriter, int, TTree, CancellationToken)"/>
    protected TreeFileWriter(TextWriter writer, TTree tree, CancellationToken cancellationToken) : this(writer, 4, tree, cancellationToken) { }

    /// <summary>
    /// Initialize an instance of <see cref="TreeFileWriter{TTree, TTreeType}"/>.
    /// </summary>
    /// <param name="tree">The tree to work for.</param>
    /// <inheritdoc cref="IndentWriter.IndentWriter(TextWriter, int, CancellationToken)"/>
    protected TreeFileWriter(TextWriter writer, int indentSize, TTree tree, CancellationToken cancellationToken) : base(writer, indentSize, cancellationToken)
    {
        _tree = tree;
