...
public class ModifySourceGenerator : AbstractSourceGenerator<ImmutableArray<AdditionalText>>
{
    protected override IncrementalValueProvider<ImmutableArray<AdditionalText>> GetRelevantInputs(IncrementalGeneratorInitializationContext context) => context.AdditionalTextsProvider.Collect();

    protected sealed override void ProduceSource(SourceProductionContext context, string? _, ImmutableArray<AdditionalText> inputs)
    {
        foreach (var input in inputs)
        {
            var fileName = Path.GetFileName(input.Path);

            if (string.Equals(fileName, "BuildClient.cs"))
                ProduceSource_BuildClient(context, input);
            else if (string.Equals(fileName, "BuildProtocol.cs"))
                ProduceSource_BuildProtocol(context, input);
            else if (string.Equals(fileName, "BuildServerConnection.cs"))
                ProduceSource_BuildServerConnection(context, input);
            else if (string.Equals(fileName, "CompilerServerLogger.cs"))
                ProduceSource_CompilerServerLogger(context, input);
        }
    }

    private void ProduceSource_BuildClient(SourceProductionContext context, AdditionalText input)
    {
        var tree = GetSyntaxTree(input, context.CancellationToken);
        if (tree is null)
            return;
    ...
    private void ProduceSource_BuildProtocol(SourceProductionContext context, AdditionalText input)
    {
        var tree = GetSyntaxTree(input, context.CancellationToken);
        if (tree is null)
            return;
    ...
    private void ProduceSource_BuildServerConnection(SourceProductionContext context, AdditionalText input)
    {
        var tree = GetSyntaxTree(input, context.CancellationToken);
        if (tree is null)
            return;
    ...
    private void ProduceSource_CompilerServerLogger(SourceProductionContext context, AdditionalText input)
    {
        var tree = GetSyntaxTree(input, context.CancellationToken);
        if (tree is null)
            return;
    ...
    private CSharpSyntaxTree? GetSyntaxTree(AdditionalText input, CancellationToken cancellationToken)
    {
        var text = input.GetText(cancellationToken);
        if (text is null)
            return null;

        return (CSharpSyntaxTree)CSharpSyntaxTree.ParseText(text, path: input.Path, cancellationToken: cancellationToken);
    }
}
