    ...
    where TTreeType : ITreeType
{
    ...
    protected static void WriteAndAddSource(SourceProductionContext context, Action<TextWriter, TTree, Compilation, CancellationToken> writeAction, TTree tree, string hintName) => AbstractSourceGenerator.WriteAndAddSource(context, writeAction, tree, hintName);
}
