    ...
    public sealed override ModuleSymbol GlobalModule
    {
        get
        {
            ...
            if (result is null)
            {
                ...
                var allGlobalModules = from m in Netmodules select m.GlobalModule;
#warning 未完成
                throw new NotImplementedException();

                result = Interlocked.CompareExchange(ref _lazyGlobalModule, result, null);
            }
    ...
    internal sealed override NamedTypeSymbol LookupDeclaredOrForwardedTopLevelMetadataType(ref MetadataTypeName emittedName, ConsList<AssemblySymbol>? visitedAssemblies)
    {
        ...
        else
        {
            ...
            // Add result of the lookup into the cache
            return CacheTopLevelMetadataType(ref emittedName, result ?? new MissingMetadataTypeSymbol.TopLevel(Netmodules[0], ref emittedName));
        }
    }
